...
def shuffle_solutions(json_data):
    # Get the current solutions mapping (person_id -> article_id)
    current_solutions = json_data["solutions"]
    
    # Create a list of all available article IDs (a through h)
    all_article_ids = [article["id"] for article in json_data["articles"]]
    
    # Create a mapping of current article IDs to their content
    article_id_to_content = {article["id"]: {"title": article["title"], "description": article["description"]} 
                             for article in json_data["articles"]}
    
    # Create a mapping from person_id to the content they need
    person_to_content = {}
    for person_id, article_id in current_solutions.items():
        person_to_content[person_id] = article_id_to_content[article_id]
    
    # Shuffle the article IDs
    shuffled_article_ids = all_article_ids.copy()
    random.shuffle(shuffled_article_ids)
    
    # Create a mapping from old article IDs to new shuffled IDs
    old_to_new_article_ids = {old_id: new_id for old_id, new_id in zip(all_article_ids, shuffled_article_ids)}
    
    # Update the articles with the new IDs and keep their content
    for i, article in enumerate(json_data["articles"]):
        old_id = article["id"]
        new_id = old_to_new_article_ids[old_id]
        json_data["articles"][i]["id"] = new_id
    
    # Update the solutions mapping
    new_solutions = {}
    for person_id, old_article_id in current_solutions.items():
        new_article_id = old_to_new_article_ids[old_article_id]
        new_solutions[person_id] = new_article_id
    
    json_data["solutions"] = new_solutions
...
def generate_mocktest():
    ...
    json_data = shuffle_solutions(json_data)
    return json_data
...
if __name__ == "__main__":
    ...
    #         with open(f"/Users/binhnguyen/Workspaces/TELC_B2_test/data_mocktest/lesen/teil_1/mocktest_generated_{idx}.json", "w", encoding="utf-8") as f:
    ...
    while idx < 100:
        try:
            mocktest = generate_mocktest()
            with open(f"/Users/binhnguyen/Workspaces/TELC_B2_test/data_mocktest/lesen/teil_1/mocktest_generated_{idx + 7}.json", "w", encoding="utf-8") as f:
                json.dump(mocktest, f, indent=4, ensure_ascii=False)
            idx += 1
        except Exception as e:
            print(f"Error generating mocktest {idx}: {e}")